...
def run():
    ...
    df = pd.read_csv(filename, header=None,
                     names=['id', 'name', 'title', 'description', 'location', 'rate', 'cat', 'skill'])
    ...
    df['country'] = df['location'].apply(
        lambda x: re.split(',', re.split(u'\xa0', x)[0])[-1].strip() if isinstance(x, str) else '')
    ...
    cPickle.dump(tfs_skills, open(file_path + "jobs_recomm_save_tfs_skill.pkl", "wb"))
    cPickle.dump(tfs_des, open(file_path + "jobs_recomm_save_tfs_des.pkl", "wb"))
    cPickle.dump(tfidf_skills, open(file_path + "jobs_recomm_save_tfidf_skill", "wb"))
    cPickle.dump(tfidf_des, open(file_path + "jobs_recomm_save_tfidf_des.pkl", "wb"))
